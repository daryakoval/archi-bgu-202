...
do_Str:                        		; do_Str function definition - functions are defined as labels
	...
	yourCode:			; use label to build a loop for treating the input string characters
		; your code goes here...
                cmp byte [ecx], 32      ;if it is Space?
                je change_char          ;go to change this char
                cmp byte [ecx], 9       ; if it is Tab?
                jnz at_finish           ;if it is not go to end of loop 
                change_char:
                        inc byte [an]         ;when changed char
			mov byte[ecx], 95
                at_finish:
